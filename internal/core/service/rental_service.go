...
type rentalService struct {
	rentalRepo      output.RentalRepository
	toolRepo        output.ToolRepository
	paymentProvider output.PaymentProvider // nil en modo desarrollo
}
...
func (s *rentalService) Create(ctx context.Context, inp input.CreateRentalInput) (*domain.Rental, error) {
	...
	if inp.CardToken != "" && s.paymentProvider != nil {
		...
		paymentID, err := s.paymentProvider.Authorize(ctx, output.AuthorizePaymentInput{
...
func (s *rentalService) GetByID(ctx context.Context, id uuid.UUID) (*domain.Rental, error) {
	return s.rentalRepo.FindByID(ctx, id)
}

func (s *rentalService) ListByUser(ctx context.Context, userID uuid.UUID) ([]*domain.Rental, error) {
...
func (s *rentalService) ConfirmDelivery(ctx context.Context, rentalID uuid.UUID, userID uuid.UUID, lat, lng float64) (*domain.Rental, error) {
...
func (s *rentalService) ConfirmReturn(ctx context.Context, rentalID uuid.UUID, userID uuid.UUID) (*domain.Rental, error) {
	rental, err := s.rentalRepo.FindByID(ctx, rentalID)
	if err != nil {
		return nil, err
	}
	...
	if updated.Status == domain.RentalStatusCompleted {
...
func (s *rentalService) Dispute(ctx context.Context, rentalID uuid.UUID, ownerID uuid.UUID, reason string) (*domain.Rental, error) {
	rental, err := s.rentalRepo.FindByID(ctx, rentalID)
	if err != nil {
		return nil, err
	}
...
func (s *rentalService) Cancel(ctx context.Context, rentalID uuid.UUID, userID uuid.UUID) (*domain.Rental, error) {
	rental, err := s.rentalRepo.FindByID(ctx, rentalID)
	if err != nil {
		return nil, err
	}
