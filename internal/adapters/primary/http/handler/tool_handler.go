...
type ToolHandler struct {
	toolSvc input.ToolService
}

func NewToolHandler(toolSvc input.ToolService) *ToolHandler {
...
func (h *ToolHandler) GetTools(c *gin.Context) {
	...
	c.JSON(http.StatusOK, dto.ToToolListResponse(tools))
}
...
func (h *ToolHandler) GetMyTools(c *gin.Context) {
	...
	filter := output.ToolFilter{OwnerID: &ownerID}
...
// @Success      200 {object} dto.ToolResponse
...
func (h *ToolHandler) GetTool(c *gin.Context) {
	...
	c.JSON(http.StatusOK, dto.ToToolResponse(tool))
}
...
// @Produce      json
// @Security     BearerAuth
// @Param        body body dto.CreateToolRequest true "Datos de la herramienta"
// @Success      201 {object} dto.ToolResponse
// @Failure      400 {object} dto.ErrResponse
// @Failure      401 {object} dto.ErrResponse
...
func (h *ToolHandler) CreateTool(c *gin.Context) {
	...
	tool, err := h.toolSvc.Create(c.Request.Context(), input.CreateToolInput{
		OwnerID:        ownerID,
		Name:           req.Name,
	...
	c.JSON(http.StatusCreated, dto.ToToolResponse(tool))
}
...
// @Security     BearerAuth
// @Param        id   path string              true "UUID de la herramienta"
// @Param        body body dto.UpdateToolRequest true "Campos a actualizar"
// @Success      200 {object} dto.ToolResponse
// @Failure      400 {object} dto.ErrResponse
// @Failure      401 {object} dto.ErrResponse
// @Failure      403 {object} dto.ErrResponse
...
func (h *ToolHandler) UpdateTool(c *gin.Context) {
	...
	tool, err := h.toolSvc.Update(c.Request.Context(), id, ownerID, input.UpdateToolInput{
		Name:           req.Name,
		Description:    req.Description,
		Category:       req.Category,
		EstimatedValue: req.EstimatedValue,
...
func (h *ToolHandler) UploadPhoto(c *gin.Context) {
	...
	tool, err := h.toolSvc.UploadPhoto(c.Request.Context(), input.UploadPhotoInput{
		ToolID:      id,
		OwnerID:     ownerID,
...
func (h *ToolHandler) GetPricingSuggestion(c *gin.Context) {
	var q dto.PricingQueryRequest
	if err := c.ShouldBindQuery(&q); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}

	suggestion := h.toolSvc.GetPricingSuggestion(q.EstimatedValue)
