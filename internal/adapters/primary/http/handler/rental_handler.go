...
type RentalHandler struct {
	rentalSvc input.RentalService
}

func NewRentalHandler(rentalSvc input.RentalService) *RentalHandler {
...
// @Produce      json
// @Security     BearerAuth
// @Param        body body dto.CreateRentalRequest true "Datos de la renta"
...
func (h *RentalHandler) CreateRental(c *gin.Context) {
	...
	rental, err := h.rentalSvc.Create(c.Request.Context(), input.CreateRentalInput{
		ToolID:      toolID,
	...
	if err != nil {
		switch {
		case errors.Is(err, service.ErrToolNotAvailable):
	...
	c.JSON(http.StatusCreated, dto.ToRentalResponse(rental))
}
...
// @Produce      json
// @Security     BearerAuth
// @Success      200 {array}  dto.RentalResponse
// @Failure      401 {object} dto.ErrResponse
// @Router       /rentals [get]
func (h *RentalHandler) GetRentals(c *gin.Context) {
	...
	c.JSON(http.StatusOK, dto.ToRentalListResponse(rentals))
}
...
// @Success      200 {object} dto.RentalResponse
// @Failure      401 {object} dto.ErrResponse
...
func (h *RentalHandler) GetRental(c *gin.Context) {
	...
	c.JSON(http.StatusOK, dto.ToRentalResponse(rental))
}
...
// @Security     BearerAuth
// @Param        id   path string                    true  "UUID de la renta"
// @Param        body body dto.ConfirmDeliveryRequest false "Coordenadas GPS (opcional)"
// @Success      200 {object} dto.RentalResponse
// @Failure      401 {object} dto.ErrResponse
...
func (h *RentalHandler) ConfirmDelivery(c *gin.Context) {
	...
	var req dto.ConfirmDeliveryRequest
	...
	if err != nil {
		...
		case errors.Is(err, domain.ErrRentalNotPending), errors.Is(err, domain.ErrAlreadyConfirmed):
			c.JSON(http.StatusConflict, gin.H{"error": err.Error()})
		default:
			handleServiceErr(c, err)
		}
...
func (h *RentalHandler) ConfirmReturn(c *gin.Context) {
	...
	if err != nil {
		...
		case errors.Is(err, service.ErrUnauthorized):
			c.JSON(http.StatusForbidden, gin.H{"error": err.Error()})
		case errors.Is(err, domain.ErrRentalNotActive), errors.Is(err, domain.ErrAlreadyConfirmed):
			c.JSON(http.StatusConflict, gin.H{"error": err.Error()})
		default:
			handleServiceErr(c, err)
		}
...
// @Security     BearerAuth
// @Param        id   path string           true "UUID de la renta"
// @Param        body body dto.DisputeRequest true "Motivo de la disputa"
// @Success      200 {object} dto.RentalResponse
// @Failure      400 {object} dto.ErrResponse
// @Failure      401 {object} dto.ErrResponse
...
func (h *RentalHandler) Dispute(c *gin.Context) {
	...
	if err != nil {
		...
		case errors.Is(err, service.ErrUnauthorized):
			c.JSON(http.StatusForbidden, gin.H{"error": err.Error()})
		case errors.Is(err, domain.ErrRentalNotActive):
			c.JSON(http.StatusConflict, gin.H{"error": err.Error()})
		default:
			handleServiceErr(c, err)
		}
...
func (h *RentalHandler) CancelRental(c *gin.Context) {
	...
	if err != nil {
		...
		case errors.Is(err, service.ErrUnauthorized):
			c.JSON(http.StatusForbidden, gin.H{"error": err.Error()})
		case errors.Is(err, domain.ErrCannotCancelDone):
			c.JSON(http.StatusConflict, gin.H{"error": err.Error()})
		default:
			handleServiceErr(c, err)
		}
