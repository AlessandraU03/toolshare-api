...
func NewToolRepository(db *pgxpool.Pool) output.ToolRepository {
	return &ToolRepository{db: db}
}

func (r *ToolRepository) Create(ctx context.Context, tool *domain.Tool) (*domain.Tool, error) {
	query := `
		INSERT INTO tools (owner_id, name, description, category, photo_url, estimated_value, daily_rate, is_available)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
		RETURNING id, owner_id, name, description, category, photo_url, estimated_value, daily_rate, is_available, created_at, updated_at
	`
	return r.scanTool(r.db.QueryRow(ctx, query,
		tool.OwnerID, tool.Name, tool.Description, tool.Category,
		tool.PhotoURL, tool.EstimatedValue, tool.DailyRate, tool.IsAvailable,
	))
}

func (r *ToolRepository) FindByID(ctx context.Context, id uuid.UUID) (*domain.Tool, error) {
	query := `
		SELECT id, owner_id, name, description, category, photo_url, estimated_value, daily_rate, is_available, created_at, updated_at
...
func (r *ToolRepository) FindAll(ctx context.Context, filter output.ToolFilter) ([]*domain.Tool, error) {
...
func (r *ToolRepository) Update(ctx context.Context, tool *domain.Tool) (*domain.Tool, error) {
	query := `
		...
		SET name = $1, description = $2, category = $3, photo_url = $4,
		    estimated_value = $5, daily_rate = $6, is_available = $7
		WHERE id = $8
		RETURNING id, owner_id, name, description, category, photo_url, estimated_value, daily_rate, is_available, created_at, updated_at
	`
	return r.scanTool(r.db.QueryRow(ctx, query,
		tool.Name, tool.Description, tool.Category, tool.PhotoURL,
		tool.EstimatedValue, tool.DailyRate, tool.IsAvailable, tool.ID,
	))
}
...
func (r *ToolRepository) scanTool(row pgx.Row) (*domain.Tool, error) {
	tool := &domain.Tool{}
	err := row.Scan(
		&tool.ID, &tool.OwnerID, &tool.Name, &tool.Description, &tool.Category,
		&tool.PhotoURL, &tool.EstimatedValue, &tool.DailyRate, &tool.IsAvailable,
