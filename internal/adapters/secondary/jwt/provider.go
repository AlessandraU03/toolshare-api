...
func NewProvider() output.TokenProvider {
	return &Provider{}
}

func (p *Provider) Generate(userID uuid.UUID, role domain.Role) (string, error) {
...
func (p *Provider) Validate(tokenStr string) (uuid.UUID, domain.Role, error) {
	secret := os.Getenv("JWT_SECRET")
	if secret == "" {
